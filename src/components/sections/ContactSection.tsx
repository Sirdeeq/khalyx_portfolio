...
const InstagramIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--text-body)] group-hover:text-white" width={20} height={20}>
...
const XIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[var(--text-body)] group-hover:text-white" width={20} height={20}>
...
export default function ContactSection() {
  ...
  return (
    <section id="contact" className="bg-[var(--bg-page)] px-5 sm:px-8 md:px-10 py-20 sm:py-24 md:py-32">
      <FadeIn y={40}>
        <h2
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
            ...
            {contactInfo.map((item) => {
              ...
              return (
                <a
                  ...
                  className="flex items-center gap-4 bg-[var(--card-bg)] backdrop-blur-xl rounded-xl border border-[var(--border-subtle)] p-4 hover:border-white/30 hover:bg-white/[0.07] transition-all duration-300 group"
                >
                  <div className="w-11 h-11 rounded-full bg-white/10 flex items-center justify-center flex-shrink-0 group-hover:bg-white/15 group-hover:scale-110 transition-all duration-300">
                    <Icon className="text-[var(--text-body)]" size={18} />
                  </div>
                  <div>
                    <p className="text-[var(--text-muted)] text-xs font-medium uppercase tracking-wider">{item.platform}</p>
                    <p className="text-[var(--text-body)] text-sm font-medium group-hover:text-white transition-colors">{item.value}</p>
                  </div>
                </a>
              )
            })}
        ...
        <FadeIn x={20} delay={0.15}>
          {sent ? (
            <div className="bg-[var(--card-bg)] backdrop-blur-xl rounded-2xl border border-[var(--border-subtle)] p-10 flex flex-col items-center justify-center text-center h-full">
              <div className="w-16 h-16 rounded-full bg-green-500/20 flex items-center justify-center mb-4">
                <span className="text-green-400 text-3xl">✓</span>
              </div>
              <h3 className="text-2xl font-bold text-[var(--text-body)] mb-2">Message Sent!</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <input
                  ...
                  className="w-full bg-[var(--card-bg)] border border-[var(--border-subtle)] rounded-xl px-5 py-4 text-[var(--text-body)] placeholder:text-[var(--text-muted-30)] outline-none focus:border-white/30 transition-colors text-sm"
                />
                <input
                  type="email"
              ...
              <select
                ...
                className="w-full bg-[var(--card-bg)] border border-[var(--border-subtle)] rounded-xl px-5 py-4 text-[var(--text-body)] outline-none focus:border-white/30 transition-colors text-sm appearance-none cursor-pointer"
              >
                <option value="" disabled className="bg-[var(--bg-page)]">
                  What are you contacting me about? *
                </option>
                {reasons.map((r) => (
                  <option key={r} value={r} className="bg-[var(--bg-page)]">
                    {r}
                  </option>
                ))}
              </select>
              ...
              <textarea
                ...
                className="w-full bg-[var(--card-bg)] border border-[var(--border-subtle)] rounded-xl px-5 py-4 text-[var(--text-body)] placeholder:text-[var(--text-muted-30)] outline-none focus:border-white/30 transition-colors text-sm resize-none"
              />
        ...
        <div className="flex flex-wrap justify-center gap-6 mt-16">
          {socialLinks.map((link) => {
            ...
            return (
              ...
              >
                <div className="w-12 h-12 rounded-full bg-[var(--card-bg)] border border-[var(--border-subtle)] flex items-center justify-center group-hover:bg-white/10 group-hover:border-white/30 group-hover:scale-110 transition-all duration-300">
                  <Icon />
                </div>
                <span className="text-[#D7E2EA]/60 text-xs group-hover:text-[var(--text-body)] transition-colors">
                  {link.handle}
                </span>
              </a>
            )
